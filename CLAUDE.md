...
- **리드 검증**: 자동추출된 정보는 신뢰도가 낮을 수 있어(마스킹, 표 파싱 오류 등) 검증상태(미확인/위반확인됨/해제확인됨) 필드로 관리. 리드 모달에서 "건축물대장 확인" 버튼을 누르면 정부24 무료 열람 검색으로 연결 — 위반건축물 여부는 건축물대장 표제부에 공식 표기됨. CODEF 같은 유료 B2B API는 "이미 아는 주소를 검증"하는 데는 쓸 수 있지만 "신규 위반건물을 발견"하는 데는 안 맞아 도입하지 않음
